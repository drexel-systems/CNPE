...
Open `lab-p1/__main__.py` and read every line. The code is complete — your task is not to write it but to understand it well enough to explain every decision in it.

As you read, ask yourself these questions (you'll answer them in writing later):

**AMI Lookup:**
```python
ami = aws.ec2.get_ami(owners=["137112412989"], filters=[...])
```
- Why specify `owners`? What risk does it mitigate?
- What is the difference between an AMI and an instance type?

**Security Group:**
```python
sec_group = aws.ec2.SecurityGroup('ssh-access', ingress=[...])
```
- AWS blocks all inbound traffic by default. Is this the right default? Why?
- The rule uses `cidr_blocks: ['0.0.0.0/0']` — what does that mean, and is it appropriate here?
- Why port 8080 instead of 80? What security principle does that reflect?

**EC2 Instance:**
```python
server = aws.ec2.Instance('lab-instance', instance_type='t4g.micro', key_name='vockey')
```
- Why `t4g.micro` specifically? What happens if you use `t3.micro` with this AMI?
- What does `key_name='vockey'` do? What would happen if you omitted it?

**Outputs:**
```python
pulumi.export('sshCommand', ...)
```
- Outputs are computed from real deployed values — not hardcoded. What does that mean for reproducibility?

> **Note:** Notice what's *missing* from this code. There's no IAM role, no outbound rule, no hardening. This is intentional — Part 1 is a baseline. As you work through Parts 2 and 3, track what gets added and why each addition is necessary.
...
**10 points total.** Capture these as you work — don't recreate them after the fact.
...
> Part 1 screenshots are deployment verification only. The analytical work for this part feeds into the written deliverables in the README — specifically the security group design questions raised in Step 1.
