...
## Step 1: Read the Part 2 Pulumi Code

Open `lab-p2/__main__.py`. Compare it to the Part 1 code. You'll notice four new things:

**New: S3 Bucket**
```python
bucket = aws.s3.BucketV2("novaspark-website", ...)
```
This creates an S3 bucket to store the HTML files. Notice `force_destroy` is NOT set here — students must empty the bucket manually before `pulumi destroy`. This is intentional: it teaches the cleanup process.

**New: IAM Role**
```python
role = aws.iam.Role("ec2-s3-read-role", assume_role_policy=...)
```
An IAM Role is a set of permissions that can be *assumed* by an AWS service. The `assume_role_policy` defines WHO can assume it — in our case, the EC2 service. The role itself does nothing until a policy is attached.

**New: IAM Role Policy**
```python
role_policy = aws.iam.RolePolicy("ec2-s3-read-policy", ...)
```
This attaches a policy to the role granting `s3:ListBucket` and `s3:GetObject` on *only* our bucket. Notably, it does NOT grant `s3:PutObject` or `s3:DeleteObject` — least privilege in action.

**New: IAM Instance Profile**
```python
instance_profile = aws.iam.InstanceProfile("ec2-instance-profile", role=role.name)
```
Here's a detail that trips up many engineers: you can't attach an IAM Role directly to an EC2 instance. EC2 requires an **Instance Profile** — a wrapper object that holds exactly one role. Think of the role as a job description and the instance profile as the ID badge that lets someone walk through the door.

**Changed: EC2 Instance**
```python
server = aws.ec2.Instance("lab2-part2-server",
    ...
    iam_instance_profile=instance_profile.name,   # <-- new!
)
```
One line change from Part 1, but it transforms what the instance can do.
...
|---|-----------------|------|---------------|--------|
| **D3** | Terminal: `aws configure list` | Step 5 | Must show `iam-role` in the `Type` column | 5 |
| **D4** | Terminal: IMDSv2 curl + JSON response | Step 5 | Must show `AccessKeyId` starting with `ASIA`, `Token`, and `Expiration` | 10 |
| **D5** | Terminal: `aws s3 ls s3://<bucket>/` | Step 6 | Must show `index.html` and `about.html` with sizes | 5 |

**Analytical deliverables (from the README):**

D6, D7, and D8 are analytical deliverables graded on depth of analysis. See the [README](../README.md) for the exact prompts and rubric.

| # | Focus | Points |
|---|-------|--------|
| **D6** | IAM role credential chain analysis + `X-Served-From: S3` as evidence | 10 |
| **D7** | Default-deny defense — threat model at scale | 10 |
| **D8** | Credential storage pattern comparison table | 10 |

> **Grader note:** D4 is the key screenshot — `ASIA` prefix confirms temporary credentials from the role, not long-lived keys. Analytical deliverables earn full credit only when grounded in specific observations from the running system — not general security knowledge.
